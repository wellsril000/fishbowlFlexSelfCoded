...
const FileUploadBox: React.FC<{ importType: string }> = ({ importType }) => {
  ...
  const isValidFileType = (file: File): boolean => {
    const validTypes = [".csv", ".xlsx", ".xls", ".xlsm"];
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (isValidFileType(file)) {
        ...
        // Navigate to the appropriate page based on file type
        ...
        setError(
          "Please select a valid file type (.csv, .xlsx, .xls, or .xlsm)"
        );
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <input
        ...
        className="hidden"
        id="file-input"
        accept=".csv,.xlsx,.xls,.xlsm" // Only allow our valid file types
      />
